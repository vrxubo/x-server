...
function _getConfig(host, pathname) {
    ...
    if(pattern){
      ...
      root = replace(host, pattern);
      if (host === 'concat.lietou-static.com') {
        root = '';
      }
    }
    ...
    if (ruleObj) {
      if (typeof ruleObj === 'string') {
        cache[original].root = cache[original].directory = decodeURI(ruleObj);
      } else if (typeof ruleObj === 'object') {
        fullName = replace(fullName, ruleObj);
        root = replace(root, ruleObj).replace(/^[^\/]*(\/.*)$/, '$1');
        cache[original].root = decodeURI(path.join(ruleObj.directory, root));
...
function readDir(pathname, dir, res) {
    fs.readdir(dir, function (err, files) {
      ...
      html = '<ul>';
      if (pathname !== '/') {
        html += '<li><a href="' + encodeURI(path.dirname(pathname).replace(/\\+/g, '/')) + '">Parent Directory</a></li>';
      }
      for (var i = 0, l = files.length; i < l; i++) {
        html += '<li><a href="' + encodeURI(path.join(pathname, files[i]).replace(/\\+/g, '/')) + '">' + files[i] + '</a></li>';
      }
      html += '</ul>';
      res.writeHead(200, {
...
function route(req, res) {
  ...
  //字体跨域
  if (/^(eot|ttf|woff|otf|svg)$/.test(filetype)) {
  ...
  if (filetype === 'shtml') {
    shtmlReader.readFile(config.root, fp, function (err, data, encoding, filetype) {
      if (err) {
        res.writeHead(404, {
          'Content-Type': 'text/plain'
        });
